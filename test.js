...
var trilateralChange_AppGL = function(width_,height_) {
	...
	kitGL_glWeb_AnimateTimer.onFrame = function(v) {
		...
		gl.clear(16384);
		_gthis.renderDraw(_gthis.penNodule.pen);
		_gthis.gl.bindBuffer(34962,_gthis.buf);
		var this1 = _gthis.penNodule.colorTriangles;
		_gthis.gl.bufferSubData(34962,0,this1.subarray(2,this1.length - 2));
...
trilateralChange_AppGL.__name__ = true;
trilateralChange_AppGL.prototype = {
	draw: function(pen) {
	}
	,renderDraw: function(pen) {
	}
};
var TrilateralRotateMove = function(width,height) {
	...
	this.dx = [];
	this.theta = 0.;
	trilateralChange_AppGL.call(this,width,height);
};
TrilateralRotateMove.__name__ = true;
TrilateralRotateMove.__super__ = trilateralChange_AppGL;
TrilateralRotateMove.prototype = $extend(trilateralChange_AppGL.prototype,{
	draw: function(pen) {
		this.curr = pen.drawType.triangleCurrent;
		var divertTrace = new kitGL_glWeb_DivertTrace();
		haxe_Log.trace("TrilateralRotateMove example",{ fileName : "TrilateralRotateMove.hx", lineNumber : 31, className : "TrilateralRotateMove", methodName : "draw"});
		var start = pen.drawType.get_pos();
		var drawType = pen.drawType;
		...
		var red = -65536;
		pen.drawType.set_pos(start);
		pen.colorType.set_pos(start);
		var color = red;
		var this1 = this.triangle;
		if(color == -1) {
			color = pen.currentColor;
		}
		pen.colorType.colorTriangles(color,this1.end - this1.start + 1);
		var green = -16711936;
		len = 0;
		start = pen.drawType.get_pos();
		var _g = 0;
		while(_g < 1000) {
			var i = _g++;
			var drawType = pen.drawType;
		...
		this.all = { start : start, end : pen.drawType.get_pos() - 1};
		var color = green;
		var this1 = this.triangle;
		if(color == -1) {
			color = pen.currentColor;
	...
	,renderDraw: function(pen) {
		...
		pen.drawType.set_pos(v);
		pen.colorType.set_pos(v);
		...
		while(_g < _g1) {
			var i = _g++;
			pen.drawType.set_pos(i);
			pen.colorType.set_pos(i);
			var _g2 = this;
...
trilateral3_nodule_PenNodule.prototype = {
	createPen: function() {
		...
		var drawAbstract = { triangle : function(ax_,ay_,az_,bx_,by_,bz_,cx_,cy_,cz_) {
			...
			return _e22[0] < dsHelper_flatInterleave_core_Flat32.get_size(_e22);
		}, get_pos : function() {
			return _e23[0];
			...
			return dsHelper_flatInterleave_core_Flat32.get_size(_e25);
		}, set_size : function(id) {
			return dsHelper_flatInterleave_core_Flat32.set_size(_e26,id);
		...
		var colorAbstract = { cornerColors : function(colorA,colorB,colorC) {
			...
			return dsHelper_flatInterleave_core_Flat32.get_size(_e39);
		}, set_size : function(id) {
			return dsHelper_flatInterleave_core_Flat32.set_size(_e40,id);
